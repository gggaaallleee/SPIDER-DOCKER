...
export const fetchSearchResults = async (query: string, pageCount: number, searchUrlBase: string) => {
  ...
  const MAX_PAGES = pageCount * 2; // 最多搜索的页面数
  ...
  while (fetchedResultsCount < pageCount && pageIndex < MAX_PAGES) {
    const searchUrl = new URL(`${searchUrlBase}?q=${encodeURIComponent(query)}&pageno=${pageIndex + 1}&format=json`);
    ...
    for (let index = 0; index < jsonResults.length; index++) {
      ...
      if (blacklistDomains.some((domain: string) => resultDomain.endsWith(domain))) {
        continue;
      }
